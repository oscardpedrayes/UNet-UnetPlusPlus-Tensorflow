...
pred_vis = [predicted_masks[17],predicted_masks[1],predicted_masks[4],predicted_masks[10],predicted_masks[7],predicted_masks[3],predicted_masks[6]]
...
with open(HISTORY_ABSOLUTE_FPATH, 'rb') as history:
    ...
    plt.ylim(0.0,1.0)
    plt.grid(True)
    plt.savefig(EXP_RESULTS + '/g_accuracy.svg')
    plt.show()
    ...
    plt.ylim(0.0,1.5)
    plt.grid(True)
    plt.savefig(EXP_RESULTS + '/loss.svg')
    plt.show()
import tensorflow as tf
...
np.savetxt(EXP_RESULTS + '/conf_mat.csv', finalConfusionMatrix, fmt='%.2f')